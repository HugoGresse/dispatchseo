...
| `builder` | Your coding agent, headless (Claude Code or Codex) - builds guides and tools, runs research (below) |
...
the stack - Claude Code or Codex, per project, whichever each site's
Settings names. Every 10 minutes it asks the backend what's due and runs it:
the daily guide build, weekly keyword research, approved tool builds, the
weekly AI-visibility scan - and on auto-mode projects it merges green guide
PRs. It only makes outbound connections (to GitHub and its agent's API -
Anthropic or OpenAI), which is why a localhost install is fully automatic
with no tunnel or domain.

Turning it on is one credential, and there are two ways to hand it over.
Both agents work the same way; the examples below show Claude Code, and the
Codex differences are one block further down.

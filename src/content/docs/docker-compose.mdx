...
create an API key in your [Cursor dashboard](https://cursor.com/dashboard)
and paste it there. Two Cursor-specific facts worth knowing before you
start: Cursor issues API keys on **paid plans only**, and the interactive
`cursor-agent login` on your own machine never reaches the builder
container - only a key does. Builds run on your Cursor plan; DispatchSEO
bills nothing.

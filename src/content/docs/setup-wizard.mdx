...
3. **Connect your coding agent** - pick Claude Code or Codex, then paste
   the credential it asks for (`claude setup-token` for Claude Code, an
   OpenAI API key for Codex). It's stored as a secret on **your repo**,
   never on our side, and builds run on your own subscription or key.
   Don't have either yet? Install
   [Claude Code](/docs/install-claude-code) or
   [Codex](/docs/install-codex) first - about five minutes, and this
   step won't work until you have.
...
One choice: **Claude Code** or **Codex**. Both do the same job here -
research, writing, pull requests - the difference is who bills you. Claude
Code runs on a Claude subscription you already pay for; Codex runs on your
own OpenAI API key, metered per run. DispatchSEO never bills you for agent
usage either way.

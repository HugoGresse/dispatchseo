...
Only if Claude Code is your agent. Claude Code isn't part of the free
Claude plan - Pro, Max, Team, and current Enterprise plans all include
it, and Pro is the cheapest way in if you're starting from nothing.
Codex is the alternative: no Claude subscription, metered per run on an
OpenAI key instead. See
...
writing happen in your coding agent, on your own plan or key -
...
Whatever your own coding agent is already configured to use. DispatchSEO
doesn't pin a model - it runs your agent, on your plan, with whatever
default or manually chosen model your agent session already has.
Change it the same way you'd change it for any other Claude Code or
Codex work.

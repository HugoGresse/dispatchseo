...
Code, an OpenAI key for Codex, or a Cursor plan's API key for Cursor (next
question); beyond that, DispatchSEO never bills you for agent usage - it
isn't a per-token service on top of your plan.
...
and so does Cursor, on its own plan's API key (see
[Use Cursor](/docs/install-cursor)).

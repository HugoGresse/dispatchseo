...
below. You bring your own coding agent - a Claude subscription for Claude
Code, or an OpenAI key for Codex (next question); beyond that, DispatchSEO
never bills you for agent usage - it isn't a per-token service on top of
your plan.
...
Only if Claude Code is your agent - Codex works too, on an OpenAI API key
instead of a subscription (see [Use Codex instead](/docs/install-codex)).
...
writing happen in your own agent - Claude Code on your subscription, or
Codex on your OpenAI key - DispatchSEO is state, scheduling, and an
approval gate around it, not a competing content engine.
...
which also covers turning it off). Hosted (dispatchseo.com): Supabase,

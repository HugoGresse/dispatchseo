...
<MetaItem label="Needs">A coding agent credential - a Claude Code OAuth token or a Codex API key (dashboard "Turn on automatic builds" step, or `CLAUDE_CODE_OAUTH_TOKEN` / `OPENAI_API_KEY` in `.env`) - without one it idles and says exactly what it's waiting for.</MetaItem>
...
image." It runs each job with your headless coding agent inside a clone of your

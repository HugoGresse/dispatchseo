...
<MetaItem label="Needs">Your project's agent credential - a Claude Code OAuth token or an OpenAI key (the credential box on Settings, or `CLAUDE_CODE_OAUTH_TOKEN` / `OPENAI_API_KEY` in `.env`) - without one it idles and says exactly what it's waiting for.</MetaItem>
...
Code or Codex, taken per job off its poll feed - inside a clone of your

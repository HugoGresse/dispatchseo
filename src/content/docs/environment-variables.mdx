...
Cursor. Create the key at
[cursor.com/dashboard/api](https://cursor.com/dashboard/api) - open that
URL directly, the page is often missing from the dashboard's own menu, but
any plan can mint one there. The interactive `cursor-agent login` on your
own machine never reaches the builder container - only a key does. Builds
draw on your Cursor plan's included usage, and nothing is billed by
DispatchSEO.
...
Named models need a paid Cursor plan - the free plan only allows `auto`.
...
<Field name="POLAR_PRODUCT_STARTER_ANNUAL" type="string" tag="Cloud only">

Optional. The Polar product id for the **yearly** Starter plan, priced at 12
times the per-month figure in `TIER_LIMITS` (`src/lib/billing.ts`). Set all
three `_ANNUAL` ids and the pricing page offers a monthly/yearly choice; leave
them unset and only the monthly plans are sold.

</Field>

<Field name="POLAR_PRODUCT_GROWTH_ANNUAL" type="string" tag="Cloud only">

Optional. The Polar product id for the yearly Growth plan.

</Field>

<Field name="POLAR_PRODUCT_SCALE_ANNUAL" type="string" tag="Cloud only">

Optional. The Polar product id for the yearly Scale plan.

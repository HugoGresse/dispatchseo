...
`OPENAI_API_KEY` below for the Codex side. The builder holds both and picks
per job, so a stack that builds some projects with each only needs both
credentials present. With neither, the builder idles and tells you so in
its logs.

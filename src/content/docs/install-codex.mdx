...
itself as a failed run at 05:13.
...
Both forms hit the same gate and see exactly the same data. Either way the key
ends up on your disk in Codex's config, the same as Claude Code stores its own.
...
calls. To stop the prompts for this server only, open your Codex config
(`~/.codex/config.toml`) and add one line under the entry the connect command
created:
...
That auto-approves DispatchSEO's tools and nothing else - every other server,
and every shell command Codex wants to run, still asks. There's no flag for it
on `codex mcp add`; the line has to be added by hand.

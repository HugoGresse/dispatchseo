...
collect over a dozen serial calls: the queue head (the approved guide to
...
with no repo change, because the workflow files carry both agents and
...
run. Ask the owner before switching: the two agents bill differently (a
Claude subscription vs metered OpenAI usage), so it's their call.

<Fields>
  <Field name="agent" type="string" required="true">
    `claude` (Claude Code, runs on a Claude subscription) or `codex`
    (Codex, metered against an OpenAI API key).
  </Field>
</Fields>
...
milestones. Call it for "what should I do about SEO today". An empty

...
**Settings → Coding agent** for the scheduled builds. You can keep using Cursor
for everything you do yourself.

## Will Cursor run the builds later?

The code for it is written and the whole chain has been run by hand: Cursor
drove the MCP server headlessly, called a tool, and came back clean. What it
hasn't done yet is run that way on a schedule, unwatched, which is a different
claim and the only one that counts.

One thing to know now, because it decides whether this will ever apply to you:
a scheduled build runs on a server with no browser, so it can't use the
`cursor-agent login` you did above. It needs an API key, and those come with a
paid Cursor plan. Connecting stays free forever; the night shift won't be.

We'll flip it on when it's been proven on a real schedule rather than in a
one-off test, and not before. A builder that quietly stops working is worse

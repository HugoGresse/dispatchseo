...
| The unattended overnight builder | Yes | Yes | **Yes** (needs a paid plan) |

The capability list is the same. One thing about that last row is not, and
it's worth knowing before you plan around it: the overnight builds run on a
server with no browser, so they can't use the login you'll do in step 1. They
need a Cursor API key, and Cursor issues those on paid plans only.

So connecting and driving Cursor by hand is free on any plan. Having it build
while you sleep is not. If you'd rather not pay for that, pick Claude Code or
Codex as the builder on **Settings → Coding agent** and keep using Cursor all
day - the builder setting only decides who runs the scheduled jobs, not who
you work with.
...
2. **A `CURSOR_API_KEY` secret on your site's repo.** Create the key in your
   Cursor dashboard (paid plans only), then either re-run the setup command

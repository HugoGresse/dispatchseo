...
pasting a Claude Code token, an OpenAI key, or a Cursor API key - each
verified before it's stored, with a green check on every agent whose key
is confirmed in place; the `?credential=` links the topbar switcher hands
you open straight onto the right tab), the
